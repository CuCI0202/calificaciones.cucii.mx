...
public class AlumnoGrupoService {
    ...
    public PageResponse<AlumnoGrupoResponse> findAll(int page, int size, Integer alumnoId,
                                                     Integer grupoId, Boolean isActive,
        ...
        long totalElements = repository.countFiltered(alumnoId, grupoId, isActive);
        int totalPages = (int) Math.ceil((double) totalElements / size);
        int offset = page * size;

        List<AlumnoGrupoResponse> content = repository.findAll(size, offset, alumnoId, grupoId, isActive, sortBy, sortDir).stream()
                .map(this::toResponse)
                .toList();
